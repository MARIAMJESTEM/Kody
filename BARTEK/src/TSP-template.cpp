...
std::vector<cost_t> CostMatrix::get_min_values_in_cols() const {
    cost_matrix_t mat = get_matrix();
    std::vector<cost_t> col = mat[0];
    std::vector<int> values;
    for(int it = 0; it<col.size(); it++){
        values.push_back(INF);
        for (int pos = 0; pos<col.size(); pos++) {
            if (mat[pos][it] <= values[it]) {
                values[it] = mat[pos][it];
            }
        }
    }
    return values;// TODO: Implement it!
}
...
std::vector<cost_t> CostMatrix::get_min_values_in_rows() const {
    ...
    for(int it = 0; it<col.size(); it++){
        values.push_back(INF);
        for (int pos = 0; pos<col.size(); pos++) {
            if (mat[it][pos] <= values[it]) {
                values[it] = mat[it][pos];
            }
        }
    }
    return values;// TODO: Implement it!
...
cost_t CostMatrix::reduce_rows() {
    std::vector<cost_t> min_rows = get_min_values_in_rows();
    int suma = 0;
    for(int i = 0; i <= min_rows.size() -1;i++){
        for (int j = 0; j <= matrix_[i].size() -1; j++){
            if (i != j ){
                matrix_[i][j] -=min_rows[i];
            }
        }
        suma += min_rows[i];
    }
...
cost_t CostMatrix::reduce_cols() {
    std::vector<cost_t> min_rows = get_min_values_in_cols();
    int suma = 0;
    for(int i = 0; i <= min_rows.size() -1;i++){
        for (int j = 0; j <= matrix_[i].size()-1; j++){
            if (i != j ){
                matrix_[j][i] -=min_rows[i];
            }
        }
        suma += min_rows[i];
    }
